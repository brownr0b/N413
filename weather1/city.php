...
if($_SESSION["user_id"] > 0){
    $url = 'https://api.openweathermap.org/data/2.5/weather?q='. $city .'&appid=65a1da2d3ac9918ebadc15f6678559d2';
    function get_http_response_code($url) {
        $headers = get_headers($url);
        return substr($headers[0], 9, 3);
    }
    if(get_http_response_code($url) == "200"){
        $sql = "INSERT INTO history_weather (`id`, `user_id`, `city`, `timestamp`) VALUES (NULL, '".$_SESSION["user_id"]."', '" . $city . "', NOW() ) ON DUPLICATE KEY UPDATE timestamp = NOW() ;";
        ...
        <nav>
            <div id="nav-left">
                ...
                <form class="form" method="get" action="city.php">
                    <input type="text" id="place" placeholder="Search for city" name="q" size="15" pattern="^[A-Za-z ,]+$" title="ex: Los Angeles; London, GB;" required>
