        ...
        $.ajax({
            ...
            success: function(data){
                ...
                $.ajax({
                    async: false,
                    global: false,
                    url: 'https://maps.googleapis.com/maps/api/timezone/json?location=' + lat + ',' + lon + '&timestamp=' + unix + '&key=AIzaSyAu6FObLrGjIixMgUJF31yxh1cvXMoXJcs',
                    success: function (data) {
                        timezone = data.timeZoneId;
                    }
                });

                var c = moment.tz((unix*1000), timezone);
                ...
                document.getElementById("citiesContainer").innerHTML +=
                    "<div class='cityInfo'>" +
                    "<div class='background'><i class='" + weatherIcon + "'></i></div>" +
                    "<div class='cityName'><a href='city.php?q=" + data.name + "' title='" + data.name + "'>" + dataName + ", " + data.sys.country + "</a></div>" +
                    ...
                    "<div class='sky'><i class='" + weatherIcon + "' title='" + data.weather[0].description.charAt(0).toUpperCase() + data.weather[0].description.slice(1).toLowerCase() + "'></i>" + data.weather[0].description.charAt(0).toUpperCase() + data.weather[0].description.slice(1).toLowerCase() + "</div>" +
    ...
    function forecast(){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                var time = moment.tz(timezone).format("h:mm A zz");

                document.getElementById("homeCurrent").innerHTML +=
                    "<div>" +
                    "<div>Home <span class='currentTime'> - " + time + "</span></div>" +
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                var threeDay = moment.tz((data.list[0].dt * 1000), timezone).format("MMMM Do");
                var threeTime = moment.tz((data.list[0].dt * 1000), timezone).format("h A");

                var sixDay = moment.tz((data.list[1].dt * 1000), timezone).format("MMMM Do");
                var sixTime = moment.tz((data.list[1].dt * 1000), timezone).format("h A");

                var nineDay = moment.tz((data.list[2].dt * 1000), timezone).format("MMMM Do");
                var nineTime = moment.tz((data.list[2].dt * 1000), timezone).format("h A");

                var twelveDay = moment.tz((data.list[3].dt * 1000), timezone).format("MMMM Do");
                var twelveTime = moment.tz((data.list[3].dt * 1000), timezone).format("h A");
                ...
                var owmThree = moment.tz((data.list[0].dt * 1000), timezone).format("H");
                var owmSix = moment.tz((data.list[1].dt * 1000), timezone).format("H");
                var owmNine = moment.tz((data.list[2].dt * 1000), timezone).format("H");
                var owmTwelve = moment.tz((data.list[3].dt * 1000), timezone).format("H");
